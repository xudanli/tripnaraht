...
exports.NaraHintService = void 0;
const common_1 = require("@nestjs/common");
let NaraHintService = class NaraHintService {
    generateNaraHint(poi) {
        var _a;
        const base = this.getBaseHint(poi.subCategory);
        const hint = { ...base };
        const append = (original, extra) => original ? `${original} ${extra}` : extra;
        if (poi.isActiveVolcano) {
            const placeName = ((_a = poi.name) === null || _a === void 0 ? void 0 : _a.primary) || '这座火山';
            hint.narrativeSeed = `在冰岛，${placeName}是一座活火山，${base.narrativeSeed}`;
            hint.reflectionHint = append(base.reflectionHint, '站在活火山前，感受地球内部的力量，思考时间尺度和人类存在的短暂。');
        }
        ...
        if (poi.elevationMeters && poi.elevationMeters > 1000) {
            hint.actionHint = append(base.actionHint, '在高海拔处，注意呼吸和保暖。');
        }
        if (poi.hazardLevel === 'high' || poi.hazardLevel === 'extreme') {
            hint.actionHint = append(hint.actionHint, '⚠️ 务必遵守现场警示标志，不要进入危险区域。');
            hint.reflectionHint = append(hint.reflectionHint, '在危险面前，更能体会到自然的威严和人类的渺小。');
        }
        if (poi.requiresGuide) {
            hint.actionHint = append(hint.actionHint, '此区域必须由专业向导带领，请提前预约。');
        }
        return hint;
    }
    getBaseHint(subCategory) {
        var _a;
        const hints = {
            volcano: {
        ...
        const base = (_a = hints[subCategory]) !== null && _a !== void 0 ? _a : hints.other;
        return { ...base };
    }
    generateNaraHints(pois) {
        const hints = new Map();
...
exports.NaraHintService = NaraHintService;
exports.NaraHintService = NaraHintService = __decorate([
    (0, common_1.Injectable)()
], NaraHintService);
//# sourceMappingURL=nara-hint.service.js.map