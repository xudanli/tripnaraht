...
function fixPackDataFormat(pack) {
    if (pack.sources) {
        pack.sources = pack.sources.map((source) => {
            if (source.type === 'json') {
                source.type = 'api';
            }
            return source;
    ...
    if (pack.rules) {
        pack.rules = pack.rules.map((rule) => {
    ...
    if (pack.hazards) {
        pack.hazards = pack.hazards.map((hazard) => {
            const typeMap = {
...
╚══════════════════════════════════════════════════════════════╝${colors.reset}\n`);
    ...
    try {
        ...
        const filePath = (0, path_1.join)(packsDir, 'pack.gl.greenland.json');
        logInfo(`保存 Pack 到文件: ${filePath}`);
        (0, fs_1.writeFileSync)(filePath, JSON.stringify(greenlandPack, null, 2), 'utf-8');
...
main();
//# sourceMappingURL=import-greenland-readiness-pack.js.map