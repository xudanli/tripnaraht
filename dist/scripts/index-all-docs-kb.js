...
function loadAllFiles(dirPath, fileList = []) {
...
function extractKeywords(content, regionName) {
...
function extractNestedText(obj, depth = 0) {
    if (depth > 10)
        return '';
...
function chunkUserPersonas(content, filename) {
    var _a;
    const chunks = [];
    const baseFilename = path.basename(filename, '.json');
    const credibilityScore = ((_a = content.metadata) === null || _a === void 0 ? void 0 : _a.credibility_score) || 0.9;
    let chunkIndex = 0;
    ...
    function extractPersonaKeywords(obj) {
        const keywords = new Set();
        const text = JSON.stringify(obj).toLowerCase();
        if (obj.persona_id)
            keywords.add(obj.persona_id);
        if (obj.persona_name)
            keywords.add(obj.persona_name.toLowerCase());
        if (obj.persona_name_en)
            keywords.add(obj.persona_name_en.toLowerCase());
        const patterns = [
    ...
    if (Array.isArray(content.user_personas)) {
        content.user_personas.forEach((persona, idx) => {
...
function autoChunk(content, filename) {
    const chunks = [];
    const baseFilename = path.basename(filename, '.json');
    let chunkIndex = 0;
...
function detectChunkType(item, filename) {
    const lowerName = filename.toLowerCase();
...
//# sourceMappingURL=index-all-docs-kb.js.map