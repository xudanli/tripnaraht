...
async function diagnoseVectors() {
    ...
    console.log('='.repeat(80));
    try {
        const dbVectors = await prisma.$queryRawUnsafe(`
      SELECT 
        c.chunk_id,
        kf.filename,
      ...
      `);
        if (dbVectors.length > 0) {
            ...
            dbVectors.forEach((v, i) => {
                ...
                const uniqueValues = new Set(vecArray.map((val) => val.toFixed(4)));
                console.log(`\n   ${i + 1}. ${v.filename}`);
                console.log(`      维度: ${v.dim}`);
                console.log(`      前10个值: ${vecArray.slice(0, 10).map((val) => val.toFixed(6)).join(', ')}`);
...
//# sourceMappingURL=diagnose-vector-issue.js.map