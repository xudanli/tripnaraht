...
function autoChunk(fileData) {
    ...
    if (fileData.filename.includes('great-walks')) {
        ...
        greatWalks.forEach((track, index) => {
            const parts = [];
            parts.push(`Great Walk名称: ${track.name || track.track_name || `Track${index}`}`);
            if (track.name_en)
                parts.push(`英文名: ${track.name_en}`);
            if (track.description)
                parts.push(`描述: ${track.description}`);
            if (track.distance_km)
                parts.push(`距离: ${track.distance_km}公里`);
            if (track.duration_days)
                parts.push(`天数: ${track.duration_days}天`);
            if (track.difficulty)
                parts.push(`难度: ${track.difficulty}`);
            if (track.booking_required)
                parts.push(`预订要求: ${track.booking_required ? '必须预订' : '可选'}`);
            chunks.push({
    ...
    if (fileData.filename.includes('day-hikes')) {
        ...
        hikes.forEach((hike, index) => {
            ...
            if (hike.name_en)
                parts.push(`英文名: ${hike.name_en}`);
            if (hike.description)
                parts.push(`描述: ${hike.description}`);
            if (hike.distance_km)
                parts.push(`距离: ${hike.distance_km}公里`);
            if (hike.duration_hours)
                parts.push(`时长: ${hike.duration_hours}小时`);
            if (hike.difficulty)
                parts.push(`难度: ${hike.difficulty}`);
            chunks.push({
    ...
    if (fileData.filename.includes('poi') || fileData.filename.includes('attraction')) {
        ...
        pois.forEach((poi, index) => {
            ...
            if (poi.nameEN)
                parts.push(`英文名: ${poi.nameEN}`);
            if (poi.description)
                parts.push(`描述: ${poi.description}`);
            if (poi.coordinates)
                parts.push(`坐标: ${poi.coordinates[0]}, ${poi.coordinates[1]}`);
            if (poi.location)
                parts.push(`位置: ${poi.location}`);
            if (poi.category)
                parts.push(`类别: ${poi.category}`);
            chunks.push({
...
//# sourceMappingURL=index-new-zealand-south-island-kb-standalone.js.map