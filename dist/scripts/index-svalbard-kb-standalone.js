...
function autoChunk(fileData) {
    ...
    if (fileData.filename.includes('attractions')) {
        ...
        attractions.forEach((item, index) => {
            ...
            if ((_a = item.overview) === null || _a === void 0 ? void 0 : _a.long_description)
                parts.push(`描述: ${item.overview.long_description}`);
            if ((_b = item.overview) === null || _b === void 0 ? void 0 : _b.short_description)
                parts.push(`简介: ${item.overview.short_description}`);
            if (item.coordinates)
                parts.push(`坐标: ${item.coordinates[0]}, ${item.coordinates[1]}`);
            if (item.region)
                parts.push(`区域: ${item.region}`);
            if (item.polar_bear_risk)
                parts.push(`北极熊风险: ${item.polar_bear_risk}`);
            if (item.highlights)
                parts.push(`亮点: ${item.highlights.join('、')}`);
            if (item.safety_notes)
                parts.push(`安全提示: ${item.safety_notes.join('、')}`);
            chunks.push({
    ...
    if (fileData.filename.includes('route')) {
        ...
        if (route.polar_bear_risk)
            parts.push(`北极熊风险: ${route.polar_bear_risk}`);
        if (route.guide_required)
            parts.push(`向导要求: ${route.guide_required}`);
        if (route.weapon_required)
            parts.push(`武器要求: ${route.weapon_required}`);
        ...
        for (const fileData of files) {
            ...
            try {
                ...
                for (let i = 0; i < chunks.length; i += batchSize) {
                    ...
                    await prisma.$transaction(chunkData.map((data) => {
...
//# sourceMappingURL=index-svalbard-kb-standalone.js.map