# Smithery TypeScript API Library

[![NPM version](<https://img.shields.io/npm/v/@smithery/api.svg?label=npm%20(stable)>)](https://npmjs.org/package/@smithery/api) ![npm bundle size](https://img.shields.io/bundlephobia/minzip/@smithery/api)

This library provides convenient access to the Smithery REST API from server-side TypeScript or JavaScript.

The REST API documentation can be found on [smithery.ai](https://smithery.ai/docs). The full API of this library can be found in [api.md](api.md).
...
await client.servers.deployments.deploy('server', {
  namespace: 'namespace',
  payload: 'payload',
...
const response = await client.health.check().catch(async (err) => {
  if (err instanceof Smithery.APIError) {
...
1. Via the `SMITHERY_LOG` environment variable
...
const client = new Smithery({
  logger: logger.child({ name: 'Smithery' }),
...
const client = new Smithery({ fetch });
```
...
```ts
import Smithery from 'npm:@smithery/api';
...
We are keen for your feedback; please open an [issue](https://www.github.com/smithery-ai/typescript-api/issues) with questions, bugs, or suggestions.
