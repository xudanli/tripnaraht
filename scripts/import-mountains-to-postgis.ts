 ...
 * 山脉数据导入脚本
 * 
 * 将 Shapefile 格式的全球山脉数据导入到 PostGIS 数据库
 * 
 * 使用方法：
 *   ts-node scripts/import-mountains-to-postgis.ts [--standard <path>] [--standard-300 <path>] [--broad <path>]
 * 
 * 示例：
 *   ts-node scripts/import-mountains-to-postgis.ts --standard data/geographic/mountains/inventory_standard/GMBA_Inventory_v2.0_standard.shp
 */
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (arg === '--standard' && args[i + 1]) {
      options.standardPath = args[i + 1];
      i++;
    } else if (arg === '--standard-300' && args[i + 1]) {
      options.standard300Path = args[i + 1];
      i++;
    } else if (arg === '--broad' && args[i + 1]) {
      options.broadPath = args[i + 1];
      i++;
  ...
  // 如果没有指定路径，尝试从默认位置查找
  const defaultStandard = path.join(process.cwd(), 'data/geographic/mountains/inventory_standard/GMBA_Inventory_v2.0_standard.shp');
  const defaultStandard300 = path.join(process.cwd(), 'data/geographic/mountains/inventory_standard_300/GMBA_Inventory_v2.0_standard_300.shp');
  const defaultBroad = path.join(process.cwd(), 'data/geographic/mountains/inventory_broad/GMBA_Inventory_v2.0_broad.shp');
  
  if (!options.standardPath && fs.existsSync(defaultStandard)) {
    options.standardPath = defaultStandard;
  }
  if (!options.standard300Path && fs.existsSync(defaultStandard300)) {
    options.standard300Path = defaultStandard300;
  }
  if (!options.broadPath && fs.existsSync(defaultBroad)) {
    options.broadPath = defaultBroad;
  }
  
  console.log('⛰️  开始导入全球山脉数据到 PostGIS\n');
  console.log('配置:');
  console.log(`  目标坐标系: EPSG:${options.srid}`);
  console.log(`  删除现有表: ${options.dropExisting ? '是' : '否'}`);
  console.log(`  标准版本: ${options.standardPath || '未指定'}`);
  console.log(`  300米版本: ${options.standard300Path || '未指定'}`);
  console.log(`  宽泛版本: ${options.broadPath || '未指定'}\n`);
  ...
  } else if (options.standard300Path) {
    // 如果没有标准版本，使用 300 米版本
    await importShapefile(
      options.standard300Path,
      'geo_mountains_standard',
      options.srid,
      options.dropExisting
    );
  } else {
    console.log('⏭️  跳过标准版本（未指定路径）');
  }
  
  // 导入 300 米版本（如果指定）
  if (options.standard300Path && options.standardPath) {
    await importShapefile(
      options.standard300Path,
      'geo_mountains_standard_300',
      options.srid,
      options.dropExisting
    );
  } else {
    console.log('⏭️  跳过 300 米版本（未指定路径或已使用为标准版本）');
  }
  
  // 导入宽泛版本（如果指定）
  if (options.broadPath) {
    await importShapefile(
      options.broadPath,
      'geo_mountains_broad',
      options.srid,
