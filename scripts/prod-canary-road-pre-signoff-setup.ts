#!/usr/bin/env npx tsx
/**
 * Step 1 — Create independent Road Canary Trip on tripnara_prod.
 *
 * Usage:
 *   ROAD_DRILL_ALLOW_PROD=1 npx tsx scripts/prod-canary-road-pre-signoff-setup.ts
...
import { arg, assertProdDatabase, today } from './prod-canary-road-pre-signoff.util';

function requireProdWrite(): void {
  if (process.env.ROAD_DRILL_ALLOW_PROD !== '1') {
    throw new Error('Set ROAD_DRILL_ALLOW_PROD=1 to write road canary trip on tripnara_prod');
  }
}
  ...
  try {
        ...
        items: [
          {
            ...
            createdBy: 'ROAD_PRE_SIGNOFF_SETUP',
            operations: [],
    ...
    await prisma.trip.upsert({
      ...
      create: {
        ...
        endDate,
        status: 'PLANNING',
        name: '[ROAD-PRE-SIGNOFF] Iceland F208 Drill',
        metadata,
        updatedAt: now,
      },
      update: {
    ...
    await prisma.itineraryItem.create({
      data: {
        ...
        note: '[ROAD-PRE-SIGNOFF] F208 highland drive',
    ...
    await prisma.itineraryItem.create({
      data: {
        ...
        note: '[ROAD-PRE-SIGNOFF] Timed activity lastEntryAt=16:00 closesAt=18:00',
    ...
    const evidence = {
      evidenceType: 'ROAD_PRE_SIGNOFF_TRIP_SETUP',
    ...
    const out = `${EVIDENCE_DIR}/prod-canary-road-setup-${today()}.json`;
