...
/**
 * 省级名称到城市名称的映射
 * 如果省级名称可以直接作为城市名称，则使用；否则需要进一步处理
 */
...
function extractCityFromAddress(address: string | null, province: string | null): string | null {
  ...
  // 尝试从地址中提取城市名称（通常在省级之后）
  // 例如："北京市通州区..." -> "北京"
  // "广东省广州市..." -> "广州"
  
  // 先尝试匹配省级
  if (province) {
    const cityName = provinceToCityMap[province];
    if (cityName) {
      // 检查地址中是否包含该城市
...
/**
 * 将等级转换为评分（用于 rating 字段）
 */
function levelToRating(level: string | null): number {
  if (!level) return 0;
 ...
 * 转换原始景点数据为 Place
 */
async function convertAttractionsToPlaces(batchSize: number = 100) {
  console.log('🔄 开始转换景点数据为 Place...\n');

  // 获取未处理的原始数据
  const total = await prisma.rawAttractionData.count({
  ...
  for (let offset = 0; offset < total; offset += batchSize) {
    const rawAttractions = await prisma.rawAttractionData.findMany({
      where: { processed: false },
      take: batchSize,
      skip: offset,
    });
    ...
    for (const raw of rawAttractions) {
      try {
        // 验证必填字段
        if (!raw.name || !raw.lat || !raw.lng) {
        ...
        const cityName = extractCityFromAddress(raw.address, raw.province) || 
                        (raw.province ? provinceToCityMap[raw.province] : null);
        ...
        const place = await prisma.place.create({
          data: {
            ...
            name: raw.name,
            category: 'ATTRACTION',
            address: raw.address || raw.encodedAddress || null,
            cityId,
            rating: levelToRating(raw.level),
            metadata: {
              level: raw.level,
        ...
        await prisma.$executeRaw`
          UPDATE "Place"
          SET location = ST_SetSRID(ST_MakePoint(${raw.lng}, ${raw.lat}), 4326)
          WHERE id = ${place.id}
        `;
    ...
    const progress = ((offset + rawAttractions.length) / total * 100).toFixed(1);
...
async function main() {
  ...
  try {
    await convertAttractionsToPlaces(batchSize);
  } catch (error: any) {
