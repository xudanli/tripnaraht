 ...
 * 海岸线数据导入脚本
 * 
 * 将 Shapefile 格式的世界海岸线数据导入到 PostGIS 数据库
 * 
 * 使用方法：
 *   ts-node scripts/import-coastlines-to-postgis.ts [--coastlines <path>]
 * 
 * 示例：
 *   ts-node scripts/import-coastlines-to-postgis.ts --coastlines data/geographic/coastlines/lines.shp
 */
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (arg === '--coastlines' && args[i + 1]) {
      options.coastlinesPath = args[i + 1];
      i++;
  ...
  // 如果没有指定路径，尝试从默认位置查找
  const defaultCoastlines = path.join(process.cwd(), 'data/geographic/coastlines/lines.shp');
  
  if (!options.coastlinesPath && fs.existsSync(defaultCoastlines)) {
    options.coastlinesPath = defaultCoastlines;
  }
  
  console.log('🌊 开始导入世界海岸线数据到 PostGIS\n');
  console.log('配置:');
  console.log(`  目标坐标系: EPSG:${options.srid}`);
  console.log(`  删除现有表: ${options.dropExisting ? '是' : '否'}`);
  console.log(`  海岸线数据: ${options.coastlinesPath || '未指定'}\n`);
  
  // 确保 PostGIS 扩展存在
  await ensurePostGISExtension();
  
  // 导入海岸线数据
  if (options.coastlinesPath) {
    await importShapefile(
      options.coastlinesPath,
      'geo_coastlines',
      options.srid,
      options.dropExisting
    );
  } else {
    console.log('⏭️  跳过海岸线数据（未指定路径）');
  }
