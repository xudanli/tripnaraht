 ...
 * - Visit Greenland Plan Your Trip (行前准备页面)
 * - Visit Greenland Brochures & Booklets (官方手册页面)
 */
...
const OFFICIAL_SOURCES = [
  {
    name: 'Visit Greenland Home',
    url: 'https://visitgreenland.com/',
    category: 'official_tourism',
    description: '格陵兰官方旅游网站主页',
    priority: 1,
  },
  {
    name: 'Visit Greenland Plan Your Trip',
    url: 'https://visitgreenland.com/plan-your-trip/',
    category: 'official_tourism',
    description: '格陵兰官方行前准备指南',
    priority: 1,
  },
  {
    name: 'Visit Greenland FAQ',
    url: 'https://visitgreenland.com/faq/',
    category: 'official_tourism',
    description: '格陵兰官方常见问题',
    priority: 1,
  },
  {
    name: 'Visit Greenland Brochures',
    url: 'https://traveltrade.visitgreenland.com/brochures-and-booklets/',
    category: 'official_resources',
    description: '格陵兰官方旅游手册与宣传资料',
    priority: 1,
  },
];
  ...
  try {
    ...
    if (structured.title) {
      chunks.push({
        ...
        section: 'overview',
        credibilityScore,
        keywords: [source.name, 'official', 'greenland', 'tourism', 'visit greenland'],
        metadata: {
          source: source.name,
          url: source.url,
...
function extractKeywords(text: string): string[] {
  ...
  // 格陵兰相关关键词
  const greenlandKeywords = [
    'greenland', 'nuuk', 'ilulissat', 'kangerlussuaq', 'visit greenland',
    'visa', 'entry', 'requirements', 'tourism', 'travel', 'visitor', 'tourist',
    'official', 'government', 'denmark', 'autonomous', 'schengen',
    'arctic', 'ice', 'glacier', 'northern lights', 'midnight sun',
    'passport', 'permit', 'adventure', 'expedition',
  ];
  
  greenlandKeywords.forEach(kw => {
  ...
  try {
    ...
    for (const source of OFFICIAL_SOURCES) {
      try {
        ...
        const filepath = `docs/greenland/official-sources/${filename}`;
