 ...
 * 河网数据导入脚本
 * 
 * 将 Shapefile 格式的河网数据导入到 PostGIS 数据库
 * 
 * 使用方法：
 *   ts-node scripts/import-rivers-to-postgis.ts [--rivers-line <path>] [--water-poly <path>] [--country <path>]
 * 
 * 示例：
 *   ts-node scripts/import-rivers-to-postgis.ts --rivers-line data/geographic/rivers/rivers_line/世界线状水系.shp
 */
...
interface ImportOptions {
  riversLinePath?: string;
  waterPolyPath?: string;
  countryPath?: string;
...
function getShapefileSRID(shpPath: string): number | null {
  ...
  try {
    ...
    // 如果没有找到 EPSG，尝试使用 ogr2ogr 或 shp2pgsql 来识别
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (arg === '--rivers-line' && args[i + 1]) {
      options.riversLinePath = args[i + 1];
      i++;
    } else if (arg === '--water-poly' && args[i + 1]) {
      options.waterPolyPath = args[i + 1];
      i++;
    } else if (arg === '--country' && args[i + 1]) {
      options.countryPath = args[i + 1];
      i++;
  ...
  // 如果没有指定路径，尝试从默认位置查找
  const defaultRiversLine = path.join(process.cwd(), 'data/geographic/rivers/rivers_line/世界线状水系.shp');
  const defaultWaterPoly = path.join(process.cwd(), 'data/geographic/rivers/water_poly/世界面状水系.shp');
  const defaultCountry = path.join(process.cwd(), 'data/geographic/rivers/country/country.shp');
  
  if (!options.riversLinePath && fs.existsSync(defaultRiversLine)) {
    options.riversLinePath = defaultRiversLine;
  }
  if (!options.waterPolyPath && fs.existsSync(defaultWaterPoly)) {
    options.waterPolyPath = defaultWaterPoly;
  }
  if (!options.countryPath && fs.existsSync(defaultCountry)) {
    options.countryPath = defaultCountry;
  }
  
  console.log('🌊 开始导入河网数据到 PostGIS\n');
  console.log('配置:');
  console.log(`  目标坐标系: EPSG:${options.srid}`);
  console.log(`  删除现有表: ${options.dropExisting ? '是' : '否'}`);
  console.log(`  线状水系: ${options.riversLinePath || '未指定'}`);
  console.log(`  面状水系: ${options.waterPolyPath || '未指定'}`);
  console.log(`  国家边界: ${options.countryPath || '未指定'}\n`);
  
  // 确保 PostGIS 扩展存在
  await ensurePostGISExtension();
  
  // 导入线状水系
  if (options.riversLinePath) {
    await importShapefile(
      options.riversLinePath,
      'geo_rivers_line',
      options.srid,
      options.dropExisting
    );
  } else {
    console.log('⏭️  跳过线状水系（未指定路径）');
  }
  
  // 导入面状水系
  if (options.waterPolyPath) {
    await importShapefile(
      options.waterPolyPath,
      'geo_water_poly',
      options.srid,
      options.dropExisting
    );
  } else {
    console.log('⏭️  跳过面状水系（未指定路径）');
  }
  
  // 导入国家边界
  if (options.countryPath) {
    await importShapefile(
      options.countryPath,
      'geo_country',
      options.srid,
