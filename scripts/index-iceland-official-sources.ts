 ...
 * - Ísland.is (https://island.is/en/p/entry)
 * - Ferðamálastofa (https://www.ferdamalastofa.is/en)
 * - Government of Iceland Tourism (https://www.government.is/topics/business-and-industry/tourism-in-iceland/)
 */
...
const OFFICIAL_SOURCES = [
  {
    ...
    description: '冰岛官方旅游网站',
    priority: 1,
  },
  {
    name: 'Ísland.is Entry Requirements',
    url: 'https://island.is/en/p/entry',
    category: 'official_visa',
    description: '冰岛政府入境要求说明',
    priority: 1,
  },
  {
    name: 'Ferðamálastofa',
    url: 'https://www.ferdamalastofa.is/en',
    category: 'official_tourism_board',
    description: '冰岛旅游局',
    priority: 1,
  },
  {
    name: 'Government of Iceland Tourism',
    url: 'https://www.government.is/topics/business-and-industry/tourism-in-iceland/',
    category: 'official_policy',
    description: '冰岛政府旅游政策',
    priority: 1,
  },
];
...
function chunkWebContent(
  ...
  const baseFilename = source.name.toLowerCase().replace(/\s+/g, '-');
  ...
  try {
    ...
    if (structured.title) {
      chunks.push({
        ...
        section: 'overview',
        credibilityScore,
        keywords: [source.name, 'official', 'iceland', 'tourism'],
        metadata: {
          source: source.name,
          url: source.url,
...
function extractKeywords(text: string): string[] {
  ...
  // 冰岛相关关键词
  const icelandKeywords = [
    'iceland', 'visa', 'entry', 'requirements', 'tourism', 'travel',
    'schengen', 'passport', 'visitor', 'tourist', 'official',
    'government', 'icelandic', 'visiticeland', 'ferdamalastofa',
  ];
  
  icelandKeywords.forEach(kw => {
  ...
  try {
    ...
    for (const source of OFFICIAL_SOURCES) {
      try {
        ...
        // 3. 保存文件记录
        const filename = `${source.name.toLowerCase().replace(/\s+/g, '-')}.json`;
        const filepath = `docs/iceland/official-sources/${filename}`;
