...
function autoChunk(fileData: any): Array<{
  ...
  if (fileData.filename.includes('activities') || fileData.filename.includes('activity')) {
    ...
    topKeys.forEach(key => {
      const sectionData = content[key];
      if (Array.isArray(sectionData)) {
        sectionData.forEach((activity: any, idx: number) => {
          const parts: string[] = [];
          if (activity.activity_id) parts.push(`活动ID: ${activity.activity_id}`);
          if (activity.name) parts.push(`活动名称: ${activity.name}`);
          if (activity.name_en) parts.push(`英文名: ${activity.name_en}`);
          if (activity.description) parts.push(`描述: ${activity.description}`);
          if (activity.difficulty) parts.push(`难度: ${activity.difficulty}`);
