 ...
 * 
 * 使用方法:
 *   npx ts-node scripts/import-ushuaia-readiness-pack.ts
 */
...
╚══════════════════════════════════════════════════════════════╝${colors.reset}\n`);
  ...
  // 乌斯怀亚 Pack 数据
  // 注意：使用 as any 来绕过类型检查，因为数据中的某些值（如 "extreme" severity, "safety_critical" category）
  // 在类型定义中可能不完全匹配，但数据本身是有效的
  const ushuaiaPack = {
  ...
  try {
    ...
    // 先保存 JSON 文件到 packs 目录（可选，用于备份）
    const packsDir = join(__dirname, '../src/trips/readiness/data/packs');
    const filePath = join(packsDir, 'pack.ar.ushuaia.json');
    ...
    writeFileSync(filePath, JSON.stringify(ushuaiaPack, null, 2), 'utf-8');
    logSuccess(`JSON 文件已保存`);
    
    // 导入到数据库（使用类型断言，因为数据中的某些枚举值可能不完全匹配类型定义）
    const result = await importPackFromJson(ushuaiaPack as ReadinessPack);
    ...
    if (result) {
      ...
      console.log(`  风险数: ${ushuaiaPack.hazards?.length || 0}`);
    } else {
      logError('导入失败！');
      process.exit(1);
    }
