...
function detectCategory(filename: string): string {
  ...
  if (filename.includes('equipment') || filename.includes('packing')) {
    return 'practical_guides';
  }
  if (filename.includes('rules') || filename.includes('laws') || filename.includes('compliance')) {
    return 'compliance_rules';
  }
  if (filename.includes('risk') || filename.includes('polar-bear') || filename.includes('weather') || filename.includes('terrain') || filename.includes('safety')) {
    return 'safety';
  }
...
function extractKeywords(item: any): string[] {
  ...
  // 提取名称字段
  ['name', 'name_cn', 'name_en', 'name_no', 'title', 'route_name', 'route_name_en'].forEach(field => {
  ...
  // 提取标签和类别
  ['tags', 'categories', 'sub_categories', 'category'].forEach(field => {
  ...
  // 提取区域和位置
  ['region', 'location', 'area'].forEach(field => {
    if (item[field] && typeof item[field] === 'string') {
      keywords.push(item[field]);
    }
  });
  ...
  // 添加斯瓦尔巴相关关键词
  keywords.push('斯瓦尔巴', 'Svalbard', '朗伊尔城', 'Longyearbyen');
...
function extractNestedText(obj: any, depth: number = 0, maxDepth: number = 3): string {
  ...
  if (Array.isArray(obj)) {
    ...
    const priorityFields = ['name', 'name_cn', 'name_en', 'title', 'description', 'overview', 
      'summary', 'content', 'details', 'intro', 'long_description', 'short_description'];
...
function autoChunk(fileData: any): Array<{
  ...
  if (fileData.filename.includes('attractions')) {
    ...
    attractions.forEach((item: any, index: number) => {
      ...
      if (item.overview?.long_description) parts.push(`描述: ${item.overview.long_description}`);
      if (item.overview?.short_description) parts.push(`简介: ${item.overview.short_description}`);
      if (item.coordinates) parts.push(`坐标: ${item.coordinates[0]}, ${item.coordinates[1]}`);
      if (item.region) parts.push(`区域: ${item.region}`);
      if (item.polar_bear_risk) parts.push(`北极熊风险: ${item.polar_bear_risk}`);
      if (item.highlights) parts.push(`亮点: ${item.highlights.join('、')}`);
      if (item.safety_notes) parts.push(`安全提示: ${item.safety_notes.join('、')}`);
      
      chunks.push({
        chunkId: `${fileData.filename}_${item.attraction_id || index}`,
        content: parts.join('\n'),
        type: 'poi',
        credibilityScore: credibility,
        keywords: extractKeywords(item),
        section: item.region,
        metadata: { file: fileData.filename, poiId: item.attraction_id },
      });
    });
    return chunks.length > 0 ? chunks : createDefaultChunks(fileData, credibility);
  }

  // 策略2: routes/*.json - 路线信息
  if (fileData.filename.includes('route')) {
    ...
    if (route.polar_bear_risk) parts.push(`北极熊风险: ${route.polar_bear_risk}`);
    if (route.guide_required) parts.push(`向导要求: ${route.guide_required}`);
    if (route.weapon_required) parts.push(`武器要求: ${route.weapon_required}`);
    if (route.best_seasons) parts.push(`最佳季节: ${route.best_seasons.join('、')}`);
...
async function indexKnowledgeBase() {
  ...
  try {
    console.log('🚀 开始索引斯瓦尔巴知识库...\n');

    // 1. 确定知识库路径
    const kbPath = process.env.KB_PATH || './docs/svalbard';
