...
} from '../src/trips/decision/evaluation/e2e-cases/iceland-highlands.example';
...
class SyntheticEngineCaptureModule {}
...
function buildWorldStateFromCase(testCase: E2ECase): TripWorldState {
  ...
  return {
    context: {
      ...
      durationDays,
      preferences: {
        intents: Object.fromEntries((testCase.input.userProfile.preferredRouteTypes ?? []).map((t) => [t, 0.8])),
        pace:
...
async function main() {
  const outDir = process.env.SYNTHETIC_ENGINE_DSO_OUT_DIR ?? 'src/trips/decision/evaluation/e2e-cases/generated/synthetic';
  ...
  const app = await NestFactory.createApplicationContext(SyntheticEngineCaptureModule, { logger: ['error', 'warn'] });
  try {
    const engine = app.get(TripDecisionEngineService);
    const outputs: Array<{ id: string; file: string }> = [];

    for (const testCase of fixtures) {
      const requestId = `synthetic-capture-${testCase.id}-${Date.now()}`;
    ...
    process.stdout.write(`Wrote ${outputs.length} synthetic fixtures with engine DSO to ${absOutDir}\n`);
  } finally {
    await app.close();
  }
}
