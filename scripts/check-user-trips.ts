  ...
  try {
    ...
    const tripsWithStatus = await prisma.trip.findMany({
      where: {
        ...
        status: 'PLANNING', // 检查 PLANNING 状态的行程
      },
      select: {
        id: true,
        destination: true,
    ...
    // 7. 检查最近的行程（包括没有 TripCollaborator 的）
    ...
    recentTrips.forEach((trip, index) => {
      const isUserTrip = trip.TripCollaborator.some(
        c => c.userId === user.id
      );
      console.log(`${index + 1}. Trip ID: ${trip.id}`);
      console.log(`   目的地: ${trip.destination}`);
