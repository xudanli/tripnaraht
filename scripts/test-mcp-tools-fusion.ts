...
async function testChat(sessionId: string, message: string): Promise<any> {
  try {
    const response = await axios.post(
      `${API_BASE}/chat`,
      {
...
async function runTest(testCase: TestCase, sessionId: string): Promise<boolean> {
...
async function main() {
  ...
  // 运行测试用例
  const results: Array<{ testCase: TestCase; passed: boolean }> = [];
