#!/usr/bin/env ts-node
/**
 * 为尼泊尔的 POI 数据添加 metadata 和 physicalMetadata
 * 
 * 功能：
 * 1. 查询所有尼泊尔的 POI（通过 regionKey (NP_*) 或坐标范围：26-31°N, 80-89°E）
...
/**
 * 查询尼泊尔的 POI（通过 regionKey 或坐标范围）
 */
async function getNepalPois() {
  const pois = await prisma.$queryRaw<Array<{
    ...
    WHERE 
      ...
      AND (
        -- 通过 regionKey 识别（尼泊尔数据通常使用 NP_* 格式）
        metadata->>'regionKey' LIKE 'NP_%'
        -- 或者通过 countryCode 识别
        OR metadata->>'countryCode' = 'NP'
        -- 或者通过坐标范围识别（尼泊尔大致范围：26-31°N, 80-89°E）
        OR (ST_Y(location::geometry) BETWEEN 26 AND 31
            AND ST_X(location::geometry) BETWEEN 80 AND 89
 ...
 */
async function enrichNepalPois() {
  console.log('🇳🇵 开始为尼泊尔 POI 数据添加 metadata 和 physicalMetadata...\n');

  try {
    // 1. 查询所有尼泊尔的 POI
    console.log('📊 查询尼泊尔 POI 数据...');
    const pois = await getNepalPois();
    console.log(`找到 ${pois.length} 个尼泊尔 POI\n`);

    if (pois.length === 0) {
      console.log('❌ 没有找到尼泊尔 POI 数据');
      return;
    }
    ...
    sampleUpdated.forEach(poi => {
      ...
      console.log(`    RegionKey: ${poi.regionKey || 'N/A'}`);
    });
