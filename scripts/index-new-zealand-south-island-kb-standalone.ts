...
function detectCategory(filename: string): string {
  ...
  if (filename.includes('equipment') || filename.includes('packing') || filename.includes('hut-booking') || filename.includes('transport')) {
    return 'practical_guides';
  }
  if (filename.includes('rules') || filename.includes('laws') || filename.includes('compliance')) {
    return 'compliance_rules';
  }
  if (filename.includes('risk') || filename.includes('weather') || filename.includes('terrain') || filename.includes('wildlife') || filename.includes('seasonal-closures')) {
    return 'safety';
  }
  if (filename.includes('route') || filename.includes('great-walks') || filename.includes('day-hikes') || filename.includes('glacier')) {
    return 'routes';
  }
  if (filename.includes('poi') || filename.includes('attraction') || filename.includes('service')) {
    return 'pois';
  }
  if (filename.includes('case-studies') || filename.includes('scenario')) {
    return 'case_studies';
  }
  return 'general';
}
...
function extractKeywords(item: any): string[] {
  ...
  // 提取名称字段
  ['name', 'name_cn', 'name_en', 'name_ma', 'title', 'route_name', 'route_name_en', 'poi_id', 'track_name'].forEach(field => {
  ...
  // 添加新西兰相关关键词
  keywords.push('新西兰', 'New Zealand', '新西兰南岛', 'South Island', 'Te Waipounamu');
...
function extractNestedText(obj: any, depth: number = 0, maxDepth: number = 3): string {
  ...
  if (Array.isArray(obj)) {
    ...
    const priorityFields = ['name', 'name_cn', 'name_en', 'name_ma', 'title', 'description', 'overview', 
...
function autoChunk(fileData: any): Array<{
  ...
  // 策略1: great-walks.json - 按Great Walk分块
  if (fileData.filename.includes('great-walks')) {
    const greatWalks = fileData.content.great_walks || fileData.content.tracks || [];
    greatWalks.forEach((track: any, index: number) => {
      const parts: string[] = [];
      parts.push(`Great Walk名称: ${track.name || track.track_name || `Track${index}`}`);
      if (track.name_en) parts.push(`英文名: ${track.name_en}`);
      if (track.description) parts.push(`描述: ${track.description}`);
      if (track.distance_km) parts.push(`距离: ${track.distance_km}公里`);
      if (track.duration_days) parts.push(`天数: ${track.duration_days}天`);
      if (track.difficulty) parts.push(`难度: ${track.difficulty}`);
      if (track.booking_required) parts.push(`预订要求: ${track.booking_required ? '必须预订' : '可选'}`);
      
      chunks.push({
        chunkId: `${fileData.filename}_${track.track_id || track.id || index}`,
        content: parts.join('\n'),
        type: 'great_walk',
        credibilityScore: credibility,
        keywords: extractKeywords(track),
        metadata: { file: fileData.filename, trackId: track.track_id || track.id },
      });
    });
    return chunks.length > 0 ? chunks : createDefaultChunks(fileData, credibility);
  }

  // 策略2: day-hikes.json - 按日间徒步分块
  if (fileData.filename.includes('day-hikes')) {
    const hikes = fileData.content.hikes || fileData.content.day_hikes || [];
    hikes.forEach((hike: any, index: number) => {
      const parts: string[] = [];
      parts.push(`徒步路线名称: ${hike.name || `Hike${index}`}`);
      if (hike.name_en) parts.push(`英文名: ${hike.name_en}`);
      if (hike.description) parts.push(`描述: ${hike.description}`);
      if (hike.distance_km) parts.push(`距离: ${hike.distance_km}公里`);
      if (hike.duration_hours) parts.push(`时长: ${hike.duration_hours}小时`);
      if (hike.difficulty) parts.push(`难度: ${hike.difficulty}`);
      
      chunks.push({
        chunkId: `${fileData.filename}_${hike.hike_id || hike.id || index}`,
        content: parts.join('\n'),
        type: 'day_hike',
        credibilityScore: credibility,
        keywords: extractKeywords(hike),
        metadata: { file: fileData.filename, hikeId: hike.hike_id || hike.id },
      });
    });
    return chunks.length > 0 ? chunks : createDefaultChunks(fileData, credibility);
  }
  ...
  if (fileData.filename.includes('route')) {
    ...
    if (route.description) parts.push(`描述: ${route.description}`);
    if (route.overview) parts.push(`概述: ${route.overview}`);
  ...
  // 策略4: pois/*.json - POI数据
  if (fileData.filename.includes('poi') || fileData.filename.includes('attraction')) {
    const pois = fileData.content.pois || fileData.content.attractions || [];
    pois.forEach((poi: any, index: number) => {
      const parts: string[] = [];
      parts.push(`POI名称: ${poi.name || poi.nameCN || `POI${index}`}`);
      if (poi.nameEN) parts.push(`英文名: ${poi.nameEN}`);
      if (poi.description) parts.push(`描述: ${poi.description}`);
      if (poi.coordinates) parts.push(`坐标: ${poi.coordinates[0]}, ${poi.coordinates[1]}`);
      if (poi.location) parts.push(`位置: ${poi.location}`);
      if (poi.category) parts.push(`类别: ${poi.category}`);
      
      chunks.push({
        chunkId: `${fileData.filename}_${poi.poi_id || poi.id || index}`,
        content: parts.join('\n'),
        type: 'poi',
        credibilityScore: credibility,
        keywords: extractKeywords(poi),
        section: poi.location || poi.region,
        metadata: { file: fileData.filename, poiId: poi.poi_id || poi.id },
      });
    });
    return chunks.length > 0 ? chunks : createDefaultChunks(fileData, credibility);
  }
...
async function indexKnowledgeBase() {
  ...
  try {
    console.log('🚀 开始索引新西兰南岛知识库...\n');

    // 1. 确定知识库路径
    const kbPath = process.env.KB_PATH || './docs/new-zealand-south-island';
    ...
    if (files.length === 0) {
      ...
      console.log('💡 提示: 请确保JSON文件已放置在以下目录:');
      console.log('   - routes/ (great-walks.json, day-hikes.json, glacier-routes.json)');
      console.log('   - pois/ (fiordland-attractions.json, queenstown-area.json, etc.)');
      console.log('   - risks/ (weather-risks.json, terrain-risks.json, etc.)');
      console.log('   - practical/ (equipment-guide.json, hut-booking-system.json, etc.)');
      console.log('   - decision-support/ (feasibility-matrix.json, rhythm-patterns.json, etc.)');
      return;
    }
