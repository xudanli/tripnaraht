 ...
 * 导入新西兰 POI 数据
 * 
 * 使用 Overpass API 从 OSM 抓取新西兰 POI 数据
 * 支持按 region 和 profile 分批导入
 * 
 * 使用方法:
 *   npm run import:nz-poi [--region <region_key>] [--profile <A|B|C|D>] [--all]
 * 
 * 示例:
 *   npm run import:nz-poi --all                    # 导入所有 region 和 profile
 *   npm run import:nz-poi --region NZ_AUCKLAND      # 只导入奥克兰
 *   npm run import:nz-poi --region NZ_AUCKLAND --profile A  # 只导入奥克兰的 Profile A
 */
...
import {
  NEW_ZEALAND_OVERPASS_PROFILES,
  buildOverpassQuery,
  type OverpassProfile,
} from './new-zealand/overpass-profiles';
import { mapOsmTagsToCanonical } from './new-zealand/canonical-mapping';
...
async function findNearestCityByLocation(
  ...
  try {
    ...
    if (nearestCity.length > 0 && nearestCity[0].distance < 150000) {
      // 如果距离小于 150 公里，认为是该城市（新西兰城市间距较大，使用更大半径）
...
function loadRegionSeeds(): RegionSeed[] {
  const seedsPath = path.join(__dirname, '../data/new-zealand/region-seeds.json');
...
/**
 * 调用 Overpass API（带重试机制，串行执行避免连接池问题）
 */
...
function convertToPlace(
  ...
  const category = tags.tourism || tags.amenity || tags.shop || tags.natural || 'other';
  
  // 映射为 canonical type
  const canonicalType = mapOsmTagsToCanonical(tags) || undefined;
...
async function importRegionProfile(
  ...
  // 根据 region 的 seed 坐标查找对应的城市（一次性查找，整个 region 共用）
  const regionCityId = await findNearestCityByLocation(region.seed.lat, region.seed.lng);
  if (regionCityId) {
    console.log(`  🏙️  匹配到城市 (cityId: ${regionCityId})`);
  } else {
    console.log(`  ⚠️  未找到匹配的城市，POI 的 cityId 将为 null`);
  }
  ...
  for (let i = 0; i < newPlaces.length; i += BATCH_SIZE) {
    ...
    try {
      // 使用事务批量插入（使用参数化查询）
      // 使用 region 级别的 cityId（已在函数开始时查找）
      await prisma.$transaction(async (tx) => {
        // 逐条插入但使用事务（更可靠，避免 SQL 注入）
        for (const place of batch) {
          // 确定 category（映射到 PlaceCategory enum）
          let placeCategory = 'ATTRACTION'; // 默认
          if (place.canonicalType === 'HUT' || place.canonicalType === 'CAMPING' || place.category === 'hotel' || place.category === 'guest_house') {
            placeCategory = 'HOTEL';
          } else if (place.canonicalType === 'SUPPLY' || place.category === 'supermarket' || place.category === 'convenience') {
            placeCategory = 'SHOPPING';
          } else if (place.canonicalType === 'AIRPORT' || place.canonicalType === 'TRANSIT' || place.canonicalType === 'FERRY_TERMINAL') {
            placeCategory = 'TRANSIT_HUB';
          } else if (place.canonicalType === 'SAFETY_MEDICAL') {
            placeCategory = 'ATTRACTION'; // 安全医疗点归类为 ATTRACTION
          }
...
async function main() {
  ...
  // 确定要处理的 regions
  const regionsToProcess = allFlag
    ? regions
    : targetRegion
  ...
  const profilesToProcess = targetProfile
    ? NEW_ZEALAND_OVERPASS_PROFILES.filter((_, i) => ['A', 'B', 'C', 'D'][i] === targetProfile.toUpperCase())
    : NEW_ZEALAND_OVERPASS_PROFILES;
  ...
  // 依次处理每个 region 的每个 profile（串行执行，避免连接池问题）
  for (const region of regionsToProcess) {
    for (const profile of profilesToProcess) {
      try {
        ...
        const delay = profile === profilesToProcess[profilesToProcess.length - 1] ? 5000 : 3000; // 最后一个 profile 后延迟更长
