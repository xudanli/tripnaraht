#!/usr/bin/env tsx
/**
 * 批量更新冰岛POI开放时间（通过API调用）
 * 
 * 调用 Places API 的批量更新接口来获取并更新开放时间
 * 
 * 使用方法:
 *   npm run script:batch-update-iceland-opening-hours
 *   或
 *   BASE_URL=http://localhost:3000 BATCH_SIZE=10 DELAY_MS=500 npm run script:batch-update-iceland-opening-hours
 */
...
const USE_API = process.env.USE_API !== 'false'; // 默认使用API，设置为false则直接调用服务
 ...
 * 调用批量更新API
 */
async function batchEnrichPlaces(placeIds: number[]): Promise<any> {
  try {
    const response = await axios.post(
      `${BASE_URL}/api/places/attractions/batch-enrich`,
      {
        placeIds,
      ...
      {
        timeout: 300000, // 5分钟超时（批量操作可能需要较长时间）
        headers: {
    ...
    if (error.response) {
      throw new Error(`API错误 (${error.response.status}): ${error.response.data?.message || error.message}`);
    }
    throw error;
  }
}
  ...
  try {
    ...
    if (result.results && Array.isArray(result.results)) {
      ...
      if (successResults.length > 0) {
        console.log(`\n✅ 成功详情（前10个）:`);
        successResults.slice(0, 10).forEach((r: any) => {
