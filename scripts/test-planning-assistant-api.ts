 ...
 * 
 * 测试接口：
 * 1. POST /api/agent/planning-assistant/sessions - 创建新会话
 * 2. POST /api/agent/planning-assistant/chat - 发送消息进行对话
 * 3. GET /api/agent/planning-assistant/sessions/:sessionId - 获取会话状态
 * 4. GET /api/agent/planning-assistant/quick-recommend - 快速推荐（无需会话）
 * 5. GET /api/agent/planning-assistant/users/:userId/preferences - 获取用户偏好摘要
 * 6. POST /api/agent/planning-assistant/users/:userId/preferences/clear - 清除用户偏好
 * 
 * 使用方法：
 *   npx tsx scripts/test-planning-assistant-api.ts [baseUrl] [userId]
 * 
 * 环境变量：
 ...
 * 测试1: 创建新会话（匿名）
 */
async function testCreateAnonymousSession(): Promise<any> {
 ...
 * 测试2: 创建新会话（用户关联）
 */
async function testCreateUserSession(): Promise<any> {
  const response = await httpRequest(
    'POST',
    `${API_BASE_URL}/api/agent/planning-assistant/sessions`,
    { userId: TEST_USER_ID }
 ...
 * 测试3: 发送消息进行对话
 */
async function testChat(sessionId: string): Promise<any> {
  const response = await httpRequest(
    'POST',
 ...
 * 测试4: 获取会话状态
 */
async function testGetSessionState(sessionId: string): Promise<any> {
  const response = await httpRequest(
    'GET',
    `${API_BASE_URL}/api/agent/planning-assistant/sessions/${sessionId}`
  );
 ...
 * 测试5: 快速推荐（无需会话）
 */
async function testQuickRecommend(): Promise<any> {
  ...
  const response = await httpRequest(
    'GET',
    `${API_BASE_URL}/api/agent/planning-assistant/quick-recommend?${params.toString()}`
  );
 ...
 * 测试6: 获取用户偏好摘要
 */
async function testGetUserPreferences(): Promise<any> {
  const response = await httpRequest(
    'GET',
 ...
 * 测试7: 清除用户偏好
 */
async function testClearUserPreferences(): Promise<any> {
  const response = await httpRequest(
    'POST',
...
async function main() {
  ...
  if (test5.success) {
    ...
    if (test5.data.recommendations && test5.data.recommendations.length > 0) {
      ...
      test5.data.recommendations.slice(0, 3).forEach((rec: any, idx: number) => {
  ...
  if (test6.success) {
    ...
    if (test6.data.topPreferences && test6.data.topPreferences.length > 0) {
      ...
      test6.data.topPreferences.slice(0, 5).forEach((pref: any, idx: number) => {
