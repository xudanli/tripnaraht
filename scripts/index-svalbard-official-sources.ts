/**
 * 斯瓦尔巴官方政府/管理机构信息源索引脚本
 * 
 * 从官方网站抓取内容并索引到RAG系统
 * 
 * 数据源：
 * - Governor of Svalbard (https://www.sysselmesteren.no/en/entry-and-residence/)
 * - Visit Svalbard (https://en.visitsvalbard.com/visitor-information/travel-information/)
 * - Field Safety Regulations (通过总督办公室网站)
 */
...
const OFFICIAL_SOURCES = [
  {
    name: 'Governor of Svalbard Entry Residence',
    url: 'https://www.sysselmesteren.no/en/entry-and-residence/',
    category: 'official_regulations',
    description: '斯瓦尔巴总督办公室 - 入境与居留规定',
    priority: 1,
  },
  {
    name: 'Visit Svalbard Travel Information',
    url: 'https://en.visitsvalbard.com/visitor-information/travel-information/',
    category: 'official_tourism',
    description: 'Visit Svalbard - 旅游信息与实用指南',
    priority: 1,
  },
  {
    name: 'Governor of Svalbard Field Safety',
    url: 'https://www.sysselmesteren.no/en/travel-in-svalbard/field-safety-regulations/',
    category: 'official_regulations',
    description: '斯瓦尔巴野外安全规定',
    priority: 1,
  },
  {
    name: 'Governor of Svalbard Travel Information',
    url: 'https://www.sysselmesteren.no/en/travel-in-svalbard/',
    category: 'official_regulations',
    description: '斯瓦尔巴旅行规定总览',
    priority: 1,
  },
];
  ...
  try {
    ...
    if (structured.title) {
      chunks.push({
        ...
        section: 'overview',
        credibilityScore,
        keywords: [source.name, 'official', 'svalbard', 'governor', 'regulations'],
        metadata: {
          source: source.name,
          url: source.url,
...
function extractKeywords(text: string): string[] {
  ...
  // 斯瓦尔巴相关关键词
  const svalbardKeywords = [
    'svalbard', 'spitsbergen', 'longyearbyen', 'governor', 'sysselmesteren',
    'visa', 'entry', 'residence', 'requirements', 'regulations', 'field safety',
    'tourism', 'travel', 'visitor', 'tourist', 'official', 'government',
    'schengen', 'norway', 'arctic', 'polar bear', 'safety', 'permit',
  ];
  
  svalbardKeywords.forEach(kw => {
...
async function indexOfficialSources() {
  try {
    console.log('🚀 开始索引斯瓦尔巴官方政府/管理机构信息源...\n');
    ...
    for (const source of OFFICIAL_SOURCES) {
      try {
        ...
        const filepath = `docs/svalbard/official-sources/${filename}`;
