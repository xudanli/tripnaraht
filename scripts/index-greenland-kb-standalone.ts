...
function detectCategory(filename: string): string {
  ...
  if (filename.includes('risk') || filename.includes('polar-bear') || filename.includes('weather') || filename.includes('terrain') || filename.includes('ice')) {
    return 'safety';
  }
  if (filename.includes('route')) {
    return 'routes';
  }
  if (filename.includes('poi') || filename.includes('attraction') || filename.includes('service') || filename.includes('settlement')) {
    return 'pois';
  }
...
function extractKeywords(item: any): string[] {
  ...
  // 提取名称字段
  ['name', 'name_cn', 'name_en', 'name_da', 'title', 'route_name', 'route_name_en'].forEach(field => {
  ...
  // 添加格陵兰相关关键词
  keywords.push('格陵兰', 'Greenland', '格陵兰岛');
...
function autoChunk(fileData: any): Array<{
  ...
  if (fileData.filename.includes('attractions') || fileData.filename.includes('pois')) {
    ...
    attractions.forEach((item: any, index: number) => {
      ...
      chunks.push({
        chunkId: `${fileData.filename}_${item.poi_id || item.attraction_id || index}`,
        content: parts.join('\n'),
        type: 'poi',
        credibilityScore: credibility,
        keywords: extractKeywords(item),
        section: item.location || item.region,
        metadata: { file: fileData.filename, poiId: item.poi_id || item.attraction_id },
      });
    });
    return chunks.length > 0 ? chunks : createDefaultChunks(fileData, credibility);
  }

  // 策略2: settlements.json - 按定居点分块
  if (fileData.filename.includes('settlement')) {
    const settlements = fileData.content.settlements || [];
    settlements.forEach((item: any, index: number) => {
      const parts: string[] = [];
      parts.push(`定居点名称: ${item.name || `定居点${index}`}`);
      if (item.name_en) parts.push(`英文名: ${item.name_en}`);
      if (item.description) parts.push(`描述: ${item.description}`);
      if (item.significance) parts.push(`重要性: ${item.significance}`);
      if (item.attractions) parts.push(`景点: ${Array.isArray(item.attractions) ? item.attractions.join('、') : item.attractions}`);
      
      chunks.push({
        chunkId: `${fileData.filename}_${item.name || index}`,
        content: parts.join('\n'),
        type: 'settlement',
        credibilityScore: credibility,
        keywords: extractKeywords(item),
  ...
  if (fileData.filename.includes('route')) {
    ...
    if (route.duration_days) parts.push(`建议天数: ${route.duration_days}`);
    if (route.difficulty_level) parts.push(`难度: ${route.difficulty_level}`);
    if (route.risk_level) parts.push(`风险等级: ${route.risk_level}`);
    if (route.guide_required) parts.push(`向导要求: ${route.guide_required ? '必须' : '可选'}`);
...
async function indexKnowledgeBase() {
  ...
  try {
    console.log('🚀 开始索引格陵兰知识库...\n');

    // 1. 确定知识库路径
    const kbPath = process.env.KB_PATH || './docs/greenland';
