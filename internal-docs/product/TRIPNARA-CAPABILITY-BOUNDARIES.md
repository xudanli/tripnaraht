...
**Shadow 研究例外（不改变对外口径）：** 允许存在 [高维隐性解析 Shadow](./LATENT-IMPLICIT-PARSE-SHADOW-CHARTER.md) 旁路 scaffold；默认关闭；输出不得写 Plan；不得据此宣称学习型 WM 已实现。
...
| `LATENT_IMPLICIT_PARSE_SHADOW` | 高维隐性解析 Shadow（默认 OFF；`1` 才启用） |
| `LATENT_IMPLICIT_PARSE_KILL_SWITCH` | 强制关闭隐性解析 Shadow |
...
- [LATENT-IMPLICIT-PARSE-SHADOW-CHARTER.md](./LATENT-IMPLICIT-PARSE-SHADOW-CHARTER.md)
