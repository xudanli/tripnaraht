...
**可选文件**：
- `全球港口数据 库.sbn/.sbx` - 空间索引（可选，提升查询性能）
- `全球港口数据 库.shp.xml` - 元数据（可选）
- `世界港口点位数 据.xls` - Excel 格式数据（需要单独处理）
...
**可选文件**：
- `T.cpg` - 代码页文件（字符编码）
- `T.sbn/.sbx` - 空间索引（可选，提升查询性能）
- `T.shp.xml` - 元数据（可选）

**注意**：航线数据可能是点数据（机场）或线数据（航线），脚本会自动识别。
...
- [港口数据指南](./ports/README.md)
- [航线数据指南](./airlines/README.md)
