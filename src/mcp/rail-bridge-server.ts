...
/**
 * Rail MCP Bridge Server
 * 
 * 本地 MCP 服务器，桥接到 Smithery 的 Rail MCP 服务
 * 允许 Claude Desktop 通过 stdio 连接使用 Rail 功能
 */
...
import { FileOAuthProvider } from './rail-client.js';
...
async function getRemoteClient(): Promise<Client> {
  ...
  const serverUrl = 'https://server.smithery.ai/DeniseLewis200081/rail';
  ...
  try {
    ...
    remoteClient = new Client({
      name: 'tripnara-rail-bridge',
      version: '1.0.0',
    });

    await remoteClient.connect(remoteTransport);
    console.error('✅ Connected to Rail MCP server (无需认证)');
    return remoteClient;
    ...
    if (needsAuth) {
      ...
      try {
        await remoteClient.connect(remoteTransport);
        console.error('✅ Connected to Rail MCP server (使用 OAuth 认证)');
        return remoteClient;
      } catch (authError) {
...
async function main() {
  ...
  console.error('✅ Rail Bridge MCP Server started');
}
