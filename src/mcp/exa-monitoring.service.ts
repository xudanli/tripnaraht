...
@Injectable()
export class ExaMonitoringService implements OnModuleInit {
  private readonly logger = new Logger(ExaMonitoringService.name);
  private readonly metricsKeyPrefix = 'exa:metrics:';
  private readonly statsKeyPrefix = 'exa:stats:';

  // Exa API 定价（基于公开信息，实际价格可能不同）
  private readonly pricing = {
    web_search_exa: 0.001, // 每次调用约 $0.001
    deep_search_exa: 0.002, // 每次调用约 $0.002
    crawling_exa: 0.0005, // 每次调用约 $0.0005
    deep_researcher_start: 0.005, // 每次启动约 $0.005
    deep_researcher_check: 0.0001, // 每次检查约 $0.0001
    default: 0.001, // 默认价格
  };
  ...
  async onModuleInit() {
    this.logger.log('ExaMonitoringService initialized');
  }

  /**
   * 记录 API 调用指标
   */
  async recordCall(metrics: ExaCallMetrics): Promise<void> {
    try {
      ...
      // 记录日志
      this.logger.debug(
        `Exa API call: ${metrics.toolName} - ${metrics.success ? 'success' : 'failed'} ` +
        `(${metrics.responseTime}ms)`
      );
    } catch (error: any) {
      this.logger.warn(`Failed to record Exa call metrics: ${error.message}`);
    }
  }
   ...
   * 更新每日统计
   */
  private async updateDailyStats(date: string, metrics: ExaCallMetrics): Promise<void> {
   ...
   * 获取每日统计
   */
  async getDailyStats(date: string): Promise<ExaDailyStats | null> {
    if (!this.redisService) {
      return null;
    }
   ...
   * 获取最近 N 天的统计
   */
  async getRecentStats(days: number = 7): Promise<ExaDailyStats[]> {
    const stats: ExaDailyStats[] = [];
   ...
   * 检查是否超过成本限制
   */
  async checkCostLimit(dailyLimit: number = 10): Promise<{
    exceeded: boolean;
    currentCost: number;
    limit: number;
  }> {
