...
/**
 * Google Maps MCP Bridge Server
 * 
 * 本地 MCP 服务器，桥接到 Smithery 的 Google Maps MCP 服务
 * 允许 Claude Desktop 通过 stdio 连接使用 Google Maps 功能
 */
...
async function getRemoteClient(): Promise<Client> {
  ...
  const serverUrl = 'https://server.smithery.ai/google_maps';
  ...
  remoteClient = new Client({
    name: 'tripnara-google-maps-bridge',
    version: '1.0.0',
  });

  try {
    await remoteClient.connect(remoteTransport);
    console.error('✅ Connected to Google Maps MCP server');
  } catch (error) {
...
async function main() {
  ...
  console.error('✅ Google Maps Bridge MCP Server started');
}
