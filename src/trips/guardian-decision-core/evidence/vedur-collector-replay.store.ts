/**
 * Anti-replay ledger for Vedur collector ingest (trip.metadata scoped).
 */
...
export const RFC001_VEDUR_COLLECTOR_INGEST_LEDGER_KEY = 'rfc001VedurCollectorIngestLedger';
...
@Injectable()
export class VedurCollectorReplayStoreService {
  ...
  private async readState(tripId: string): Promise<IngestLedgerState> {
    ...
    const raw = (trip?.metadata as Record<string, unknown> | null)?.[
      RFC001_VEDUR_COLLECTOR_INGEST_LEDGER_KEY
  ...
  private async writeState(tripId: string, state: IngestLedgerState): Promise<void> {
    ...
    meta[RFC001_VEDUR_COLLECTOR_INGEST_LEDGER_KEY] = state;
