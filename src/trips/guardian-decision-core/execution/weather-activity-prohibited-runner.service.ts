/**
 * Slice 2 — weather activity: evidence → problem → workspace → Decision Core finalize.
 */
...
export interface WeatherActivityProhibitedRunResult {
  runId: string;
  tripId: string;
...
@Injectable()
export class WeatherActivityProhibitedRunnerService {
  private readonly logger = new Logger(WeatherActivityProhibitedRunnerService.name);

  constructor(
    private readonly pipeline: WeatherActivityProhibitedPipelineService,
    private readonly evaluateService: WeatherActivityProhibitedEvaluateService,
  ...
  ): Promise<WeatherActivityProhibitedRunResult> {
  ...
  private emptyRun(tripId: string): WeatherActivityProhibitedRunResult {
    return {
      runId: '',
      tripId,
      problem: null,
