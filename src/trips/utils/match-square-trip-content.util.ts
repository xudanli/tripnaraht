...
type MatchSquareInstantiationMeta = {
  strategy?: TripInstantiationStrategy;
  routeDirectionName?: string | null;
};

function readInstantiation(metadata: unknown): MatchSquareInstantiationMeta | null {
  if (!metadata || typeof metadata !== 'object') return null;
  const inst = (metadata as Record<string, unknown>).matchSquareInstantiation;
  if (!inst || typeof inst !== 'object') return null;
  return inst as MatchSquareInstantiationMeta;
}
  ...
  switch (strategy) {
    ...
    default:
      return {
        status: 'completed',
        stage: 'skeleton_only',
        message: '搭子成团骨架已创建，请补充路线模板或发起日程编排',
        itemsCount: itemCount,
...
/** 对历史搭子行程在读取时补齐 generationProgress（不落库） */
...
/** 历史搭子实例化行程缺少 generationProgress 时需回填 */
