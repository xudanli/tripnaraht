...
| 修复抽屉 | POST | `/api/readiness/repair-options` | `data.cascadeUiHints` + **`data.guardianNegotiation`**（见下方契约） |
...
**repair-options 响应 — `data.guardianNegotiation`（2026-06-15）**：

优先读 `trip.metadata` 中同 `blockerId` 的 `preRepair`；若无则运行 `pre_repair` 三人格博弈（需 `READINESS_GUARDIAN_NEGOTIATION≠0` 且 DecisionModule 可用）。前端 mock 可关：`VITE_READINESS_CASCADE_MOCK=false`。

```typescript
guardianNegotiation?: {
  consensus?: 'ALIGNED' | 'SPLIT' | 'BLOCKED';
  summary?: string;
  personas: Array<{
  ...
  userActionRequired?: string[];  // snake_case: user_action_required
  analyzedAt?: string;             // snake_case: analyzed_at
};
```
