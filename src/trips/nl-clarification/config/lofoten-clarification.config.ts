// src/trips/nl-clarification/config/lofoten-clarification.config.ts
// 罗弗敦目的地澄清配置

import { DestinationClarificationConfig } from './destination-clarification.config';
import { LOFOTEN_USER_PERSONAS } from './lofoten-personas.config';

export const LOFOTEN_CONFIG_TEMPLATE: DestinationClarificationConfig = {
  destinationCode: 'LF', // 罗弗敦（使用2字符代码）
  destinationName: '罗弗敦',
  enabled: true,
  metadata: {
  ...
  userPersonas: LOFOTEN_USER_PERSONAS,
  clarificationRounds: [
    {
    ...
    },
    {
      ...
      triggerConditions: {
        ...
        // 🆕 不要求上一轮完成，因为round_1_basic的问题为空，由LLM自动提取基础字段
