...
describe('booking-cart-checkout.util', () => {
  it('update_selection 校验 slot 唯一性', async () => {
  ...
  it('update_selection 重算 within_budget', async () => {
    const cart = sampleCart();
    const res = await applyBookingCartAction({
      cart,
      action: 'update_selection',
  ...
  it('apply_saving 换选更便宜航班', async () => {
    let cart = sampleCart();
    cart = (
      await applyBookingCartAction({
      cart,
      action: 'update_selection',
      payload: { selected_item_ids: ['flight_leg0_rank1', 'c1'] },
    })
  ...
  it('confirm_ready 超预算需 acknowledge', async () => {
    const cart = (
      await applyBookingCartAction({
      cart: sampleCart(),
      action: 'update_selection',
    ...
    const rejected = await applyBookingCartAction({
      cart,
      action: 'confirm_ready',
    });
    expect(rejected.status).toBe('REJECTED');

    const ok = await applyBookingCartAction({
      cart,
      action: 'confirm_ready',
  ...
  it('submit_checkout 需先 confirm_ready 并返回 Bundle', async () => {
    const cart = sampleCart();
    const rejected = await applyBookingCartAction({ cart, action: 'submit_checkout' });
    expect(rejected.status).toBe('REJECTED');

    const ready = (await applyBookingCartAction({ cart, action: 'confirm_ready' })).booking_cart;
    const submitted = await applyBookingCartAction({ cart: ready, action: 'submit_checkout' });
    ...
    expect(submitted.checkout?.bundle?.schema).toBe('tripnara.booking_checkout_bundle@v1');
