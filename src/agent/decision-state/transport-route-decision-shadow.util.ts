/**
 * Transport / Route Decision State Shadow
 */

import { getTransportRouteDecisionContract } from './transport-route-decision.contracts';
...
export type BuildTransportRouteDecisionShadowInput = {
  message: string;
...
export function buildTransportRouteDecisionShadow(
  input: BuildTransportRouteDecisionShadowInput,
): DecisionStateShadowV1 {
  const tripId = input.hints?.tripId ?? null;
  const classified = classifyTransportRouteDecision(input.message, tripId);
  const contract = classified.decisionClass
    ? getTransportRouteDecisionContract(classified.decisionClass)
    : null;
