...
describe('POST /agent/route_and_run — Weather Wind Lock strict Sentinel auto-heal (E2E)', () => {
  ...
  it('blocks DRIVE under extreme wind and auto-heals to stay-in-place (C1 strict)', async () => {
    ...
    expect(Array.isArray(secondReq?.emergency_constraints?.forbidden_modes)).toBe(true);
    expect(secondReq?.emergency_constraints?.forbidden_modes).toEqual(expect.arrayContaining(['DRIVE', 'MOTORCYCLE']));
    expect(Array.isArray(secondReq?.emergency_constraints?.preferred_modes)).toBe(true);
