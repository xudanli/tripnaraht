...
import { resolveItineraryAdjustRunContext } from '../../../utils/itinerary-adjust-decision-log.util';
...
export function appendVerifyTemporalOpeningAuditProof(
  ...
  try {
    ...
    const targetDate = adjustCtx.active ? adjustCtx.targetDateIso?.slice(0, 10) : undefined;
    for (const day of state.itinerary.days ?? []) {
      const dayDate = String(day.date ?? '').slice(0, 10);
      if (targetDate && dayDate !== targetDate) continue;
