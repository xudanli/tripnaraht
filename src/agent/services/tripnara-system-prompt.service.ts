...
/**
 * TripNARA System Prompt Service
 * 
 * 提供 TripNARA Agent 的系统提示（人格 + 世界观 + 决策宪法）
 * 用于在 LLM 调用时注入统一的 Agent 人格
 */
...
@Injectable()
export class TripNaraSystemPromptService {
...
- **Abu（保守）**: 保护核心体验，避免失败，优先稳定
- **Dr.Dre（结构调整）**: 拆天，调节节奏，优化顺序
- **Neptune（修复）**: 换入口，替代 POI，局部重构
...
替用户在复杂、陌生、不可逆的现实世界中，做出负责任的旅行决策。`;
   ...
   * 提取精简版（用于 token 限制场景）
   */
  private extractCompactVersion(): string {
