...
| cascadeMode | enum | 否 | **级联调整模式**。`auto`: 自动调整后续行程项（默认）；`none`: 只调整当前项，不影响后续 | `"auto"` |
...
控制更新行程项时间时是否自动调整后续行程项：

- **`auto`**（默认）：自动调整后续行程项
  - 更新 `startTime` 时，系统会根据实际距离和交通方式计算旅行时间
  - 自动调整后续所有行程项的时间，保持时间顺序
  - 会检测并报告级联影响，需要用户确认
  
- **`none`**：只调整当前项，不影响后续
  - 更新 `startTime` 时，只更新当前行程项的时间
  - 不调整后续行程项，可能导致时间重叠或顺序问题
  - 不检测级联影响，直接更新

**使用场景**：
- `auto`: 需要保持整个行程时间顺序一致时使用（推荐）
- `none`: 只想微调单个行程项，且确定不会影响后续时使用

### 智能时间调整功能说明

当更新 `startTime` 且 `cascadeMode` 为 `auto`（默认）时，系统会执行以下智能调整：
