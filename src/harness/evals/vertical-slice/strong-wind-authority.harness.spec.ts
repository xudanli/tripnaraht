...
describe('strong-wind authority vertical slice', () => {
  ...
  it('strong-wind writeback uses real UWC Preview → Confirm → Apply', async () => {
    ...
    const tripId = 'trip_is_south_coast_uwc';
    ...
    const preview = await controller.preview({
      ...
      tripId,
      intendedMutation: {
        request_id: 'r_wind_vs',
        context_signature: 'sig_wind_strong',
        decision_scope_trigger: 'WEATHER_DETERIORATION_STRONG_WIND',
      },
      expectedWriteVersion: {
        kind: 'RESOURCE_VERSION_SET',
        resources: [{ resourceId: tripId, expectedVersion: 3 }],
      },
    });
    ...
    expect(confirm.confirmationId).toBeTruthy();

    const prevCorridor = process.env.UWC_CORRIDOR_MODE_ACTIONS_COMMIT;
    ...
    try {
      const applied = await controller.apply({
        ...
        idempotencyKey: `idem_wind_${tripId}`,
        productSurface: 'web',
      });
      expect(applied.stage).toBe('APPLY');
      expect(applied.applyPipelineStages.length).toBe(7);
      expect(UWC_WRITE_APPLY_PATH).toBe('POST /api/uwc/v1/write/apply');
    } finally {
      if (prevCorridor === undefined) {
        delete process.env.UWC_CORRIDOR_MODE_ACTIONS_COMMIT;
      } else {
        process.env.UWC_CORRIDOR_MODE_ACTIONS_COMMIT = prevCorridor;
      }
    }
  });
});
